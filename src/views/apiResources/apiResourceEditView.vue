<template>
  ...
  <Form
    @submit="onSubmit"
    :validation-schema="apiResourceSchema"
    autocomplete="off"
    ref="myform"
  >
    ...
    <div class="form">
      <label for="allowedAccessTokenSigningAlgorithms" class="form-label">Allowed Access Token Signing Algorithms</label>
      <Field
        id="allowedAccessTokenSigningAlgorithms"
        name="allowedAccessTokenSigningAlgorithms"
        type="text"
        class="form-control"
        v-model="model.allowedAccessTokenSigningAlgorithms"
      />
      <ErrorMessage name="allowedAccessTokenSigningAlgorithms" class="form-text" />
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      emptyModel: {
        ...
        allowedAccessTokenSigningAlgorithms: false,
        nonEditable: false,
      },
      model: {},
    };
  ...
  methods: {
    ...
    validateForm() {
      return this.apiResourceSchema.validate(this.model, { abortEarly: false });
    },
    async reset() {
      if (this.isNew) {
        this.model = JSON.parse(JSON.stringify(this.emptyModel));
    ...
    async onSubmit(values) {
      try {
        ...
        this.SpinnerService.show();
        values.id = this.apiResourceId;

        await this.ApiResourceService.addUpdateApiResource(values);
  ...
  computed: {
    ...
    isValid() {
      try {
        this.apiResourceSchema.validateSync(this.model);
        return true;
      } catch {
        //ignore
      }
