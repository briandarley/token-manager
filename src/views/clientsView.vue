    ...
    <table class="table table-condensed">
       <thead class="thead-primary">
        ...
        <tr v-for="entity in entities" :key="entity.id">
            <td>{{entity.id}}</td>
            <td>{{entity.name}}</td>
            <td>{{entity.grantTypes}}</td>
            <td>
                <router-link :to="{name: 'client-edit', params: {id: entity.id}}">edit</router-link>
...
<script>
export default {
  dependencies: ['ConfigReaderService','ClientsService'],
  ...
  methods: {
    async initialize() {
        this.entities = await this.ClientsService.getClients();
    }
  }
