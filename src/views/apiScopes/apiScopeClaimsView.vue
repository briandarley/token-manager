<template>
  ...
  <div>
    ...
    >
      <div class="card">
        <div class="card-header">Add Api Scope Claim</div>
        <div class="card-body">
  ...
  methods: {
    ...
    async loadModel() {
      ...
      try {
        ...
        this.entities = apiScopes.userClaims;
      } catch (error) {
    ...
    async deleteEntity(entity) {
      try {
        this.SpinnerService.show();
        await this.ApiScopesService.deleteApiScopeClaim(entity);
        await this.loadModel();
        await this.reset();
        this.ToastService.success("Successfully Deleted API Scope Claim");
      } catch (error) {
        this.ToastService.error(error);
      } finally {
        this.SpinnerService.hide();
    ...
    async onSubmit(values) {
      ...
      try {
        ...
        await this.ApiScopesService.addUpdateApiScopeClaim(values);
        await this.loadModel();
        await this.reset();

        this.ToastService.success("Successfully Added New API Scope Claim");
      } catch (error) {
        this.ToastService.error(error);
      } finally {
        this.SpinnerService.hide();
